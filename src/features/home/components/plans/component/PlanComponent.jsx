    ...
    <div className="max-w-5xl mx-auto px-4 py-12 text-white" >
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {plans.map((plan, index) => (
          <div key={index} className={`border border-[#29292a] rounded-lg bg-[#1f1f1f] hover:border-[#3a3a3a] transition-all duration-300 ${plan.popular ? 'relative ring-1 ring-[#b7b7b9]' : ''} ${index === 0 ? 'py-4' : 'py-6'}`}>
            ...
            <div className={`${index === 0 ? 'px-6 ' : 'px-6'}`}>
              <button className={`w-full py-2 px-4 rounded-full text-sm font-medium transition-all duration-300 ${
                plan.popular 
